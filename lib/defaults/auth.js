...
const encrypt = Promise.coroutine(function * (text) {
...
const decrypt = Promise.coroutine(function * (token) {
...
const addUser = Promise.coroutine(function * (unsafeToken, email) {
  ...
  try {
    token = yield encrypt(unsafeToken);
  } catch (e) {
    throw (e);
  }
 ...
 *    User-provided email address
 *
 * @return {Promise that must resolve with the token's username or with false}
 */
const verifyUser = Promise.coroutine(function * (token) {
  let dec;
  try {
    dec = yield decrypt(token);
  } catch (e) {
    throw (e);
  }
