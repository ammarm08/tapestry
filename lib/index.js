...
app.get('/:package/:version?', wrap(req, res, next) => {
...
app.get('/:package/-/:filename', wrap(req, res, next) => {
  // npm install
...
app.get('/-/all/:anything?', wrap(req, res, next) => {
  // TBD: search packages?
});

app.post('/_session', Cookies.express(), wrap(req, res, next) => {
...
app.get('/-/user/:db_user', wrap(req, res, next) => {
  return res.status(200).json({ok: `you are authenticated as "${req.remote_user.name}"`});
});

app.put('/-/user/:db_user/:rev?/:revision?', wrap(req, res, next) => {
  // create token
  let token;
...
app.put('/:package/:rev?/:revision?', wrap(req, res, next) => {
...
app.delete('/:package/-rev/*', wrap(req, res, next) => {
  // npm unpublish <package-name>
});

app.delete('/:package/-/:filename/-rev/:revision', wrap(req, res, next) => {
  // npm unpublish  <tarball-name>
});

app.put('/:package/-/:filename/*', wrap(req, res, next) => {
  // npm publish <tarball>
});
